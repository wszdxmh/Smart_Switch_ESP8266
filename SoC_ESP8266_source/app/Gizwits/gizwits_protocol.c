...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    /* 只处理可写bool Enum类型数据 */
    if(0x01 == aFlag->flagswitchcontrol)
    {
        bitFieldBitLen += SWITCHCONTROL_LEN;
    }
...
static int32_t ICACHE_FLASH_ATTR gizCompressValue(uint32_t bitOffset,uint32_t bitLen,uint8_t *bufAddr,uint32_t srcData)
{
    uint8_t highBit = 0;
    uint8_t lowBit = 0;
    uint8_t ret = 0;
...
*/
static int32_t ICACHE_FLASH_ATTR gizDecompressionValue(uint32_t bitOffset,uint32_t bitLen,uint8_t *arrayAddr,uint32_t arrayLen)
{
    int32_t destValue = 0;
    uint8_t ret = 0;
    uint32_t byteOffset = 0;
    uint8_t highBit = 0 ,lowBit = 0;
    uint8_t destBufTemp[COUNT_W_BIT];//临时BUF
...
{
    uint32_t bitFieldByteLen= 0;//位段字节长度
    uint32_t bitFieldOffset = 0;//位段位偏移
    uint32_t byteFieldOffset = 0;//字节段偏移

    gizwitsElongateP0Form_t elongateP0FormTmp;
    memset((uint8_t *)&elongateP0FormTmp,0,sizeof(gizwitsElongateP0Form_t));
    memcpy((uint8_t *)&elongateP0FormTmp.devDatapointFlag,issuedData,DATAPOINT_FLAG_LEN);
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagswitchcontrol)
    {
        info->event[info->num] = EVENT_SWITCHCONTROL;
        info->num++;
        dataPoints->valueswitchcontrol = gizDecompressionValue(bitFieldOffset,SWITCHCONTROL_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += SWITCHCONTROL_LEN;
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtimeon_hour)
    {
        info->event[info->num] = EVENT_TIMEON_HOUR;
        info->num++;
        memcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetimeon_hour,(uint8_t *)&issuedData[byteFieldOffset],TIMEON_HOUR_LEN);
        byteFieldOffset += TIMEON_HOUR_LEN;
        dataPoints->valuetimeon_hour = gizX2Y(TIMEON_HOUR_RATIO,  TIMEON_HOUR_ADDITION, elongateP0FormTmp.devStatus.valuetimeon_hour); 
    }




    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtimeon_min)
    {
        info->event[info->num] = EVENT_TIMEON_MIN;
        info->num++;
        memcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetimeon_min,(uint8_t *)&issuedData[byteFieldOffset],TIMEON_MIN_LEN);
        byteFieldOffset += TIMEON_MIN_LEN;
        dataPoints->valuetimeon_min = gizX2Y(TIMEON_MIN_RATIO,  TIMEON_MIN_ADDITION, elongateP0FormTmp.devStatus.valuetimeon_min); 
    }




    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtime_delay)
    {
        info->event[info->num] = EVENT_TIME_DELAY;
        info->num++;
        memcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetime_delay,(uint8_t *)&issuedData[byteFieldOffset],TIME_DELAY_LEN);
        byteFieldOffset += TIME_DELAY_LEN;
        dataPoints->valuetime_delay = gizX2Y(TIME_DELAY_RATIO,  TIME_DELAY_ADDITION, elongateP0FormTmp.devStatus.valuetime_delay); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueswitchcontrol != cur->valueswitchcontrol)
    {
        GIZWITS_LOG("valueswitchcontrol Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagswitchcontrol = 1;
        ret = 1;
    }
    if(last->valuetimeon_hour != cur->valuetimeon_hour)
    {
        GIZWITS_LOG("valuetimeon_hour Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtimeon_hour = 1;
        ret = 1;
    }
    if(last->valuetimeon_min != cur->valuetimeon_min)
    {
        GIZWITS_LOG("valuetimeon_min Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtimeon_min = 1;
        ret = 1;
    }
    if(last->valuetime_delay != cur->valuetime_delay)
    {
        GIZWITS_LOG("valuetime_delay Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtime_delay = 1;
        ret = 1;
    }
...
{
    uint32_t bitFieldByteLen= 0;//位段字节大小
    uint32_t bitFieldOffset = 0;//位偏移
    uint32_t byteFieldOffset = 0;//字节偏移

    uint8_t allDatapointByteBuf[sizeof(gizwitsElongateP0Form_t)];//开辟最大数据点空间
    memset(allDatapointByteBuf,0,sizeof(gizwitsElongateP0Form_t));

    memcpy(allDatapointByteBuf,(uint8_t *)&gizwitsProtocol.waitReportDatapointFlag,DATAPOINT_FLAG_LEN);
    if(DATAPOINT_FLAG_LEN > 1)
    {
        gizByteOrderExchange(allDatapointByteBuf,DATAPOINT_FLAG_LEN);
    }
    byteFieldOffset += DATAPOINT_FLAG_LEN;//先偏移出Flag大小的位置
        
    devStatus_t devStatusTmp;//临时设备数据点变量
    ...
    /*** 填充位段 ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagswitchcontrol)
    {
        gizCompressValue(bitFieldOffset,SWITCHCONTROL_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueswitchcontrol);
        bitFieldOffset += SWITCHCONTROL_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagtimeon_hour)
    {
        devStatusTmp.valuetimeon_hour = gizY2X(TIMEON_HOUR_RATIO,  TIMEON_HOUR_ADDITION, dataPoints->valuetimeon_hour); 
        memcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetimeon_hour,TIMEON_HOUR_LEN);
        byteFieldOffset += TIMEON_HOUR_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtimeon_min)
    {
        devStatusTmp.valuetimeon_min = gizY2X(TIMEON_MIN_RATIO,  TIMEON_MIN_ADDITION, dataPoints->valuetimeon_min); 
        memcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetimeon_min,TIMEON_MIN_LEN);
        byteFieldOffset += TIMEON_MIN_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtime_delay)
    {
        devStatusTmp.valuetime_delay = gizY2X(TIME_DELAY_RATIO,  TIME_DELAY_ADDITION, dataPoints->valuetime_delay); 
        memcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetime_delay,TIME_DELAY_LEN);
        byteFieldOffset += TIME_DELAY_LEN;
    }
    ...
    memset((uint8_t *)&gizwitsProtocol.waitReportDatapointFlag,0,DATAPOINT_FLAG_LEN);//清空标志位
    *outDataLen = byteFieldOffset;
    memcpy(outData,allDatapointByteBuf,*outDataLen);
    return 0;
}
    ...
    switch(inData[0]) 
    {
        ...
        case ACTION_READ_DEV_STATUS:
            ...
            if(0 == gizDataPoints2ReportData(&gizwitsProtocol.gizLastDataPoint,gizwitsProtocol.reportData,(uint32_t *)&gizwitsProtocol.reportDataLen))
            {
                ...
                os_memcpy(&outData[1], (uint8_t *)&gizwitsProtocol.reportData, gizwitsProtocol.reportDataLen);
...
void ICACHE_FLASH_ATTR gizTask(os_event_t * events)
{
    ...
    case SIG_IMM_REPORT:
        ...
        if(0 == gizDataPoints2ReportData(&gizwitsProtocol.gizLastDataPoint,gizwitsProtocol.reportData,(uint32_t *)&gizwitsProtocol.reportDataLen))
        {
            ...
            os_memcpy((uint8_t *)&gizwitsP0Max.data, (uint8_t *)&gizwitsProtocol.reportData, gizwitsProtocol.reportDataLen);
            gagentUploadData((uint8_t *)&gizwitsP0Max, gizwitsProtocol.reportDataLen + 1);
        }
        break;
    default:
        os_printf("---error sig! ---\n");
